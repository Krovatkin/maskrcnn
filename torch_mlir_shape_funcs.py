# This script is used to get the list of shape functions
# implemented in JIT and torch-mlir.
# This is based on https://github.com/llvm/torch-mlir/blob/578d0ec292e629c967553fa454383defc5642e65/python/torch_mlir/dialects/torch/importer/jit_ir/build_tools/shape_lib_gen.py#L808
# and torch-mlir parts are removed so one can easily run the script and
# get the list of shape functions w/o building the entire torch-mlir
...
def check_shape_function(*args, **kwargs):
    def wrap(f):
        return f
...
def get_shape_functions():
    ...
    return [x.replace("aten〇", "") for x in names if '〇' in x]


if __name__== "__main__":
    print(get_shape_functions())
    print(f"Number of shape functions {len(get_shape_functions())}")